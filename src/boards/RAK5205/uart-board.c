...
UartContext_t UartContext[UART_3 + 1]; // indexed by UART_n

extern Uart_t ConsoleUart;  // UART_1
extern Uart_t GpsUart;     // UART_3
...
USART3_IRQHandler(void)
{
    ...
    // UART in mode Transmitter end
    if ((tmpFlag != RESET) && (tmpItSource != RESET)) {
        if ((UartContext[UART_3].UartHandle.State == HAL_UART_STATE_BUSY_RX) ||
